...
public final class PixelProtectCommand {
    ...
    public LiteralArgumentBuilder<CommandSourceStack> create() {
        return Commands.literal("pixelprotect")
                ...
                .then(Commands.literal("inspect").requires(s -> s.getSender().hasPermission("pixelprotect.inspect"))
                        .executes(ctx -> inspect(ctx.getSource().getSender())))
                .then(Commands.literal("lookup").requires(s -> s.getSender().hasPermission("pixelprotect.lookup"))
                        .then(Commands.argument("radius", IntegerArgumentType.integer(1, maxRadius))
                                .then(Commands.argument("hours", IntegerArgumentType.integer(1, maxHours))
                                        .executes(ctx -> lookup(ctx.getSource(), IntegerArgumentType.getInteger(ctx, "radius"), IntegerArgumentType.getInteger(ctx, "hours"), ""))
                                        .then(Commands.argument("selectors", StringArgumentType.greedyString())
                                                .executes(ctx -> lookup(ctx.getSource(), IntegerArgumentType.getInteger(ctx, "radius"), IntegerArgumentType.getInteger(ctx, "hours"), StringArgumentType.getString(ctx, "selectors")))))))
                ...
                .then(Commands.literal("rollback").requires(s -> s.getSender().hasPermission("pixelprotect.rollback"))
                        .then(Commands.argument("radius", IntegerArgumentType.integer(1, maxRadius))
                                .then(Commands.argument("hours", IntegerArgumentType.integer(1, maxHours))
                                        ...
                                        .then(Commands.argument("selectors", StringArgumentType.greedyString())
                                                .executes(ctx -> rollback(ctx.getSource(), IntegerArgumentType.getInteger(ctx, "radius"), IntegerArgumentType.getInteger(ctx, "hours"), StringArgumentType.getString(ctx, "selectors")))))))
    ...
    private void sendLookup(CommandSender sender, List<AuditEntry> entries) {
        ...
        for (AuditEntry entry : entries.stream().limit(15).toList()) send(sender, "#" + entry.id() + " " + entry.actorName() + " " + entry.action() + " @ " + entry.x() + "," + entry.y() + "," + entry.z());
