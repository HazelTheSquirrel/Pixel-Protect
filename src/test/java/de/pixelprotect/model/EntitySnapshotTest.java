...
class EntitySnapshotTest {
    @Test void roundTripsStructuredSnapshot(){
        UUID id=UUID.randomUUID(); EntitySnapshot original=new EntitySnapshot(id,"minecraft:zombie","Zombie",0,64,0,0,0,0,0,0,0,0,0,false,false,false,false,true,true,null,null);
        EntitySnapshot parsed=EntitySnapshot.parse(original.serialize());
        assertNotNull(parsed); assertEquals(original.uuid(),parsed.uuid()); assertEquals(original.type(),parsed.type());
    }
