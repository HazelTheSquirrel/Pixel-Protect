...
public final class BlockAuditListener implements Listener {
    ...
    @EventHandler(priority=EventPriority.MONITOR,ignoreCancelled=true) public void onFluid(BlockFromToEvent e){if(!fluids||!isFluid(e.getBlock().getType()))return;Block b=e.getToBlock();BlockSnapshot before=BlockSnapshot.capture(b);later(b,()->audit.recordEnvironment(b,ActionType.FLUID,before,BlockSnapshot.capture(b)));}
