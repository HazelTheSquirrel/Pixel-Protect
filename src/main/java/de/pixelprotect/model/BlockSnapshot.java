...
public record BlockSnapshot(String blockData, byte[] inventory, String blockEntity) {
    ...
    public static BlockSnapshot fromState(BlockState state, BlockData data) {
        byte[] inventory = state instanceof InventoryHolder holder ? ItemStack.serializeItemsAsBytes(holder.getInventory().getContents()) : null;
    ...
    private static String serializeBlockEntity(BlockState state) {
        ...
        if (state instanceof Sign sign) {
            o.addProperty("kind", "sign"); o.addProperty("waxed", sign.isWaxed());
            o.add("front", serializeSide(sign.getSide(Side.FRONT))); o.add("back", serializeSide(sign.getSide(Side.BACK)));
        } else if (state instanceof Skull skull) {
            o.addProperty("kind", "skull"); var profile = skull.getProfile();
            ...
            o.addProperty("kind", "spawner"); o.addProperty("spawnedType", spawner.getSpawnedType() == null ? null : spawner.getSpawnedType().getKey().toString());
            o.addProperty("delay", spawner.getDelay()); o.addProperty("minDelay", spawner.getMinSpawnDelay()); o.addProperty("maxDelay", spawner.getMaxSpawnDelay());
            o.addProperty("maxNearby", spawner.getMaxNearbyEntities()); o.addProperty("requiredRange", spawner.getRequiredPlayerRange());
            o.addProperty("spawnRange", spawner.getSpawnRange()); o.addProperty("spawnCount", spawner.getSpawnCount());
        } else if (state instanceof Container) return null;
        return o.isEmpty() ? null : GSON.toJson(o);
    }
    private static JsonObject serializeSide(SignSide side) {
        JsonObject o = new JsonObject(); JsonArray lines = new JsonArray();
    ...
    public static void applyBlockEntity(BlockState state, String data) {
        if (data == null) return;
        try {
            JsonObject o = JsonParser.parseString(data).getAsJsonObject(); String kind = o.get("kind").getAsString();
            if (state instanceof Sign sign && kind.equals("sign")) {
                applySide(sign.getSide(Side.FRONT), o.getAsJsonObject("front")); applySide(sign.getSide(Side.BACK), o.getAsJsonObject("back"));
                sign.setWaxed(o.get("waxed").getAsBoolean()); sign.update(true, false);
            } else if (state instanceof CreatureSpawner spawner && kind.equals("spawner")) {
                if (o.has("spawnedType") && !o.get("spawnedType").isJsonNull()) spawner.setSpawnedType(org.bukkit.Registry.ENTITY_TYPE.get(org.bukkit.NamespacedKey.fromString(o.get("spawnedType").getAsString()))); else spawner.setSpawnedType(null);
                spawner.setDelay(o.get("delay").getAsInt()); spawner.setMinSpawnDelay(o.get("minDelay").getAsInt()); spawner.setMaxSpawnDelay(o.get("maxDelay").getAsInt());
                spawner.setMaxNearbyEntities(o.get("maxNearby").getAsInt()); spawner.setRequiredPlayerRange(o.get("requiredRange").getAsInt()); spawner.setSpawnRange(o.get("spawnRange").getAsInt()); spawner.setSpawnCount(o.get("spawnCount").getAsInt()); spawner.update(true, false);
            }
    ...
    private static void applySide(SignSide side, JsonObject json) {
        ...
        side.setGlowingText(json.get("glowing").getAsBoolean()); side.setColor(org.bukkit.DyeColor.valueOf(json.get("color").getAsString()));
    }
}
