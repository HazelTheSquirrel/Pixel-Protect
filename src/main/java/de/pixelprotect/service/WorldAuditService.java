...
public final class WorldAuditService {
    ...
    public void entityPlace(EntityPlaceEvent e){if(ForensicGuard.active())return;if(!(e.getEntity() instanceof InventoryHolder h))return;Entity entity=e.getEntity();Player p=e.getPlayer();ownership.record(EndpointResolver.resolve(h.getInventory()),new Owner(p.getUniqueId(),p.getName()));}
