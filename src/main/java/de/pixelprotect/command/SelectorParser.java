...
public final class SelectorParser {
    ...
    public static Parsed parse(List<String> tokens, int defaultRadius, int defaultHours, int maxRadius, int maxHours) {
        ...
        String user = null, world = null;
        Integer x = null, y = null, z = null, chunkX = null, chunkZ = null;
        ...
        for (String token : tokens) {
            ...
            String raw = value.substring(colon + 1).trim();
            try {
                switch (key) {
                    case "u" -> user = raw;
                    ...
                    case "w" -> world = raw;
                    case "c" -> {
                        int[] p = parseCoordinates(raw);
                        x = p[0]; y = p[1]; z = p[2]; chunkX = chunkZ = null;
                    }
                    case "ch", "chunk" -> {
                        String[] p = raw.split(",", -1);
                        if (p.length != 2) throw new IllegalArgumentException("Chunk muss x,z sein.");
                        chunkX = Integer.parseInt(p[0].trim());
                        chunkZ = Integer.parseInt(p[1].trim());
                        x = y = z = null;
                    }
                    default -> errors.add("Unbekannter Selektor: " + key + ":");
                }
            } catch (IllegalArgumentException e) {
                errors.add(e.getMessage() == null ? "Ungültiger Selektor: " + value : e.getMessage());
            }
        }
        ...
        return new Parsed(radius, durationMillis, user, world, x, y, z, chunkX, chunkZ,
                includeActions, excludeActions, includeBlocks, excludeBlocks, count, preview, page, errors);
    }
    ...
    private static int[] parseCoordinates(String raw) {
        ...
        try {
            ...
            throw new IllegalArgumentException("Ungültige Koordinaten: " + raw);
        }
    }
    ...
    private static void addValues(String raw, Set<String> target) {
        for (String part : raw.split(",", -1)) {
            String value = part.trim().toLowerCase(Locale.ROOT);
            if (value.isEmpty() || !value.matches("[a-z0-9_./:-]+")) throw new IllegalArgumentException("Ungültiger Blockfilter: " + part.trim());
    ...
    private static long parseDurationMillis(String raw) {
        ...
        if (value.isEmpty()) throw new IllegalArgumentException("Zeitangabe darf nicht leer sein.");
        try {
            if (value.matches("\\d+(?:\\.\\d+)?")) return Math.round(Double.parseDouble(value) * 3_600_000D);
            if (value.matches("\\d+(?:\\.\\d+)?[smhdw]")) {
                ...
                double millis = number * multiplier;
                if (!Double.isFinite(millis) || millis > Long.MAX_VALUE) throw new ArithmeticException();
                return Math.round(millis);
            }
            throw new IllegalArgumentException("Zeitangabe muss z.B. 30s, 30m, 12h oder 7d sein.");
