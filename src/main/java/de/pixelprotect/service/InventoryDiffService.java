...
/** Deterministic slot-level inventory diff plus human-readable forensic item snapshots. */
public final class InventoryDiffService {
    ...
    /** Returns every occupied slot from a persisted inventory snapshot, preserving slot positions. */
    public static List<SlotItem> occupiedSlots(byte[] bytes) {
        ...
        for (int slot = 0; slot < items.length; slot++) {
            ...
            if (item != null && !item.isEmpty() && item.getAmount() > 0) {
                result.add(new SlotItem(slot, item));
            }
        }
    ...
    static boolean matchesEncoded(String[] current, String expectedDiff, boolean compareAfter) {
        ...
        try {
            ...
            if (root == null || !root.has("changes") || !root.get("changes").isJsonArray()) return false;
            ...
            for (var element : changes) {
                ...
                if (!expected.equals(valueAt(current, slot))) return false;
            }
            return true;
    ...
    private static List<ItemChange> itemChangesFromArrays(ItemStack[] before, ItemStack[] after) {
        ...
        for (Aggregate aggregate : totals.values()) {
            if (aggregate.amount == 0 || aggregate.sample == null) continue;
    ...
    private static String[] encodeAll(ItemStack[] items) {
        ...
        for (int slot = 0; slot < items.length; slot++) encoded[slot] = encode(items[slot]);
        return encoded;
    }

    private static String valueAt(String[] values, int slot) {
        return values != null && slot >= 0 && slot < values.length && values[slot] != null ? values[slot] : "";
    }
